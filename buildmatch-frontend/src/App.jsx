  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: `linear-gradient(135deg, ${C.primary} 0%, ${C.primaryDark} 100%)`, padding: "28px 20px 40px", borderBottomLeftRadius: 28, borderBottomRightRadius: 28 }}>
        ...
        <div style={{ background: C.white, borderRadius: 14, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
...
const ClientSearch = ({ query: initQ, onProfSelect }) => {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: C.white, padding: "20px 16px 24px", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ background: C.lightGray, borderRadius: 12, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10 }}>
          <Icon icon={faSearch} size={16} color={C.gray} />
          <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Enter" && search(q)}
            placeholder="Pesquisar profissionais..." style={{ background: "transparent", border: "none", outline: "none", flex: 1, fontSize: 14, fontFamily: "'DM Sans', sans-serif", color: C.dark }} />
          <button onClick={() => search(q)} style={{ background: C.primary, color: "#fff", border: "none", padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans', sans-serif" }}>Buscar</button>
        </div>
      </div>
      <div style={{ padding: 16 }}>
        <Card style={{ padding: 14, marginBottom: 16 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: C.dark, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {loading ? <Spinner /> : profs.length === 0
          ? <div style={{ textAlign: "center", padding: 48, color: C.gray }}><Icon icon={faSearch} size={48} color={C.border} /><p style={{ marginTop: 12 }}>Nenhum profissional encontrado</p></div>
...
const ClientProfile = ({ user, onLogout }) => {
  ...
  const BackBtn = ({ light }) => (
    <button onClick={() => setSection(null)} style={{ background: light ? "rgba(255,255,255,0.15)" : C.lightGray, border: "none", borderRadius: 10, padding: "8px 14px", cursor: "pointer", fontSize: 14, fontFamily: "'DM Sans', sans-serif", color: light ? "#fff" : C.dark, display: "flex", alignItems: "center", gap: 6 }}>
      <Icon icon={faArrowLeft} size={13} color={light ? "#fff" : C.dark} /> Voltar
    </button>
  );
      ...
      <div style={{ padding: "20px 16px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ position: "relative", display: "inline-block" }}>
            ...
            <button style={{ position: "absolute", bottom: 0, right: 0, background: C.primary, border: "none", width: 30, height: 30, borderRadius: "50%", cursor: "pointer", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Icon icon={faPencilAlt} size={12} color="#fff" />
            </button>
          </div>
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: "0 16px", marginTop: -24 }}>
        ...
        <p style={{ textAlign: "center", color: C.gray, fontSize: 12, marginBottom: 16 }}>BuildMatch v1.0.0 — Universidade de Santiago</p>
...
const ProfPortfolio = ({ user }) => {
  ...
  useEffect(() => {
    ...
    import("./services/api").then(({ portfolioAPI }) =>
      portfolioAPI.list(user.professional.id).then(d => setItems(d.data || [])).catch(() => setItems([])).finally(() => setL(false))
    );
  }, []);
    ...
    <div style={{ padding: "20px 16px", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <Btn onClick={() => setShowForm(!showForm)} variant="accent" small>
          <Icon icon={showForm ? faTimes : faPlus} size={12} color="#fff" /> {showForm ? "Cancelar" : "Adicionar"}
        </Btn>
      </div>
      {showForm && (
        <Card style={{ marginBottom: 16, borderTop: `4px solid ${C.accent}` }}>
          <Input label="Título" value={title} onChange={e => setTitle(e.target.value)} placeholder="Ex: Moradia T3 em Assomada" />
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Descreva o projecto..."
          ...
          <Btn onClick={add} variant="accent" full disabled={saving || !title}>
            <Icon icon={faSave} size={13} color="#fff" /> {saving ? "A guardar..." : "Guardar projecto"}
          </Btn>
        </Card>
      )}
      {loading ? <Spinner /> : items.length === 0
        ? <div style={{ textAlign: "center", padding: 48, color: C.gray }}><Icon icon={faImages} size={48} color={C.border} /><p style={{ marginTop: 12 }}>Sem itens no portfólio</p></div>
        : items.map((item, i) => (
          <Card key={i} style={{ marginBottom: 12 }}>
            <div style={{ height: 120, background: `linear-gradient(135deg, ${C.primary}20, ${C.accent}20)`, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
  ...
  const EditProfProfile = () => {
    ...
    const save = async () => {
      ...
      try {
        ...
        if (prof.id) {
          await professionalsAPI.update(prof.id, { specialty, location, about, available, priceMin: parseFloat(priceMin) || null, priceMax: parseFloat(priceMax) || null, experience: parseInt(experience) || 0, tags });
        }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: C.primary, padding: "20px 16px 24px", borderBottomLeftRadius: 24, borderBottomRightRadius: 24 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#fff", fontSize: 16, cursor: "pointer", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
          <Icon icon={faArrowLeft} size={14} color="#fff" /> Voltar
        </button>
